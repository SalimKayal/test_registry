...
# create persistent local environment
local_env=${HOME}/work/.envs/project
if [ -d ${local_env} ]; then
    tmpfile=$(mktemp)
    mamba list --explicit > $tmpfile
    mamba install -y -p ${local_env} --update-specs --file $tmpfile 
    rm $tmpfile
else
    mamba create -p ${local_env} --clone project
fi
